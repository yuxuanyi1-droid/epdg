...
@ns_auc.route('/swm/eap_aka/plmn/<string:plmn>/imsi/<string:imsi>')
class PyHSS_SWm_Get_EAP_AKA_Vectors(Resource):
    def get(self, imsi, plmn):
        '''Get SWm-ready EAP-AKA vectors for specified IMSI and PLMN'''
        try:
            auc_data = databaseClient.Get_AuC(imsi=imsi)
            plmn = diameterClient.EncodePLMN(mcc=plmn[0:3], mnc=plmn[3:])
            vector_dict = databaseClient.Get_Vectors_AuC(auc_data['auc_id'], action='eap_aka', plmn=plmn)
            return {'allowed': True, 'imsi': imsi, 'plmn': plmn, 'vectors': vector_dict}, 200
        except Exception as E:
            print(E)
            return handle_exception(E)
