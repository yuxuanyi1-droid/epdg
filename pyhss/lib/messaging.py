...
class RedisMessaging:
    ...
    def awaitBulkMessage(self, key: str, count: int=100, usePrefix: bool=False, prefixHostname: str='unknown', prefixServiceName: str='common'):
        ...
        try:
            key = self.handlePrefix(key=key, usePrefix=usePrefix, prefixHostname=prefixHostname, prefixServiceName=prefixServiceName)
            try:
                return self.redisClient.blmpop(0, 1, key, direction='RIGHT', count=count)
            except Exception:
                # Older Redis servers do not support BLMPOP.
                # Fall back to BLPOP and return a shape similar to BLMPOP:
                # [key, [msg1, msg2, ...]]
                first = self.redisClient.blpop(key)
                if first is None:
                    return ''
                queue_name, message = first
                messages = [message]
                for _ in range(max(count - 1, 0)):
                    next_message = self.redisClient.lpop(key)
                    if next_message is None:
                        break
                    messages.append(next_message)
                return [queue_name, messages]
        except Exception as e:
