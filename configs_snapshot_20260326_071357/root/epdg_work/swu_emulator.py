...
class swu():
    ...
    def __init__(self, source_address,epdg_address,apn,modem,default_gateway,mcc,mnc,imsi,ki,op,opc,netns,sqn,ims_options=None):
        ...
        self.sqn = sqn
        self.ims_options = ims_options or {}
...
### USER PLANE FUNCTIONS AND INTER PROCESS COMMUNICATION ####
    ...
    def run_ims_actions(self):
        if not self.ims_options.get('enabled'):
            return 0
        if not getattr(self, 'ip_address_list', None):
            print('IMS actions skipped: no UE inner IP assigned yet')
            return 1

        local_ip = self.ip_address_list[0]
        pcscf_ip = self.ims_options.get('pcscf_ip')
        if not pcscf_ip and getattr(self, 'pcscf_address_list', None):
            if self.pcscf_address_list:
                pcscf_ip = self.pcscf_address_list[0]
        if not pcscf_ip:
            print('IMS actions skipped: no P-CSCF address available from CP or options')
            return 1

        if self.netns_name and pcscf_ip != self.server_address[0]:
            cleanup_cmd = "ip route del %s/32 2>/dev/null || true" % pcscf_ip
            self.exec_in_netns(cleanup_cmd)

        realm = self.ims_options.get('realm') or ('ims.mnc' + self.mnc + '.mcc' + self.mcc + '.3gppnetwork.org')
        register_domain = self.ims_options.get('register_domain') or 'ims.localdomain'
        public_id = self.ims_options.get('public_id') or self.imsi
        private_id = self.ims_options.get('private_id') or (self.imsi + '@' + realm)
        modem = self.com_port or DEFAULT_COM
        actions = self.ims_options.get('actions') or 'register'
        sec_prot = self.ims_options.get('sec_prot') or 'esp'
        sec_mode = self.ims_options.get('sec_mode') or 'trans'
        sec_alg = self.ims_options.get('sec_alg') or 'hmac-md5-96'
        sec_ealg = self.ims_options.get('sec_ealg') or 'null'
        script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'ims_sip_register.py')
        cmd = [
            sys.executable,
            script_path,
            '--local-ip', local_ip,
            '--pcscf-ip', pcscf_ip,
            '--pcscf-port', str(self.ims_options.get('pcscf_port', 5060)),
            '--imsi', self.imsi,
            '--realm', realm,
            '--register-domain', register_domain,
            '--public-id', public_id,
            '--private-id', private_id,
            '--contact-port', str(self.ims_options.get('contact_port', 6062)),
            '--sec-port-c', str(self.ims_options.get('sec_port_c', 6062)),
            '--sec-port-s', str(self.ims_options.get('sec_port_s', 6063)),
            '--sec-prot', sec_prot,
            '--sec-mode', sec_mode,
            '--sec-alg', sec_alg,
            '--sec-ealg', sec_ealg,
        ...
        if self.ims_options.get('invite_target'):
            cmd.extend(['--invite-target', self.ims_options.get('invite_target')])
        if self.ki:
            cmd.extend(['--ki', self.ki])
        if self.op:
            cmd.extend(['--op', self.op])
        if self.opc:
            cmd.extend(['--opc', self.opc])
        if self.netns_name:
            cmd = ['ip', 'netns', 'exec', self.netns_name] + cmd

        print('\nRunning IMS actions:', ' '.join(shlex.quote(part) for part in cmd), '\n')
        return subprocess.run(cmd).returncode

    def run_ims_actions_async(self):
        result = self.run_ims_actions()
        if result != 0:
            print('IMS actions failed')
...
#### STATES ####
    ...
    def state_connected(self):
        ...
        self.ike_to_ipsec_encoder.send(self.encode_inter_process_protocol(inter_process_list_start_encoder))
        self.ike_to_ipsec_decoder.send(self.encode_inter_process_protocol(inter_process_list_start_decoder))
        if self.ims_options.get('enabled'):
            ims_worker = multiprocessing.Process(target=self.run_ims_actions_async)
            ims_worker.start()
...
def main():
    ...
    parser.add_option("--ims-register", action="store_true", dest="ims_register", default=False, help="Run IMS SIP tests automatically after SWu tunnel is established")
    parser.add_option("--ims-actions", dest="ims_actions", default="register", help="Comma-separated IMS actions: register,re-register,deregister,invite")
    parser.add_option("--ims-pcscf-ip", dest="ims_pcscf_ip", help="Override P-CSCF IP for IMS tests")
    parser.add_option("--ims-pcscf-port", dest="ims_pcscf_port", type="int", default=5060, help="P-CSCF SIP port for IMS tests")
    parser.add_option("--ims-realm", dest="ims_realm", help="IMS realm for SIP registration")
    parser.add_option("--ims-register-domain", dest="ims_register_domain", default="ims.localdomain", help="REGISTER request domain")
    parser.add_option("--ims-public-id", dest="ims_public_id", help="IMS public identity user part")
    parser.add_option("--ims-private-id", dest="ims_private_id", help="IMS private identity")
    parser.add_option("--ims-contact-port", dest="ims_contact_port", type="int", default=6062, help="UE SIP contact port")
    parser.add_option("--ims-sec-port-c", dest="ims_sec_port_c", type="int", default=6062, help="Security-Client port-c")
    parser.add_option("--ims-sec-port-s", dest="ims_sec_port_s", type="int", default=6063, help="Security-Client port-s / UE receive port")
    parser.add_option("--ims-invite-target", dest="ims_invite_target", help="Target SIP URI for INVITE action")
    ...
    ims_options = {
        'enabled': options.ims_register,
        'actions': options.ims_actions,
        'pcscf_ip': options.ims_pcscf_ip,
        'pcscf_port': options.ims_pcscf_port,
        'realm': options.ims_realm,
        'register_domain': options.ims_register_domain,
        'public_id': options.ims_public_id,
        'private_id': options.ims_private_id,
        'contact_port': options.ims_contact_port,
        'sec_port_c': options.ims_sec_port_c,
        'sec_port_s': options.ims_sec_port_s,
        'invite_target': options.ims_invite_target,
    }

    a = swu(options.source_addr,destination_addr,options.apn,options.modem,options.gateway_ip_address,options.mcc,options.mnc,options.imsi,options.ki,options.op,options.opc,options.netns, options.sqn, ims_options)
