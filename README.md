...
```bash
python3 -m pyepdg ./configs/epdg.yaml
```

开发配置只保留本地编排：

```bash
python3 -m pyepdg ./configs/epdg.dev.yaml
```
...
- `configs/epdg.yaml` 和 `configs/epdg.std.yaml` 默认指向 PyHSS。
- `configs/epdg.dev.yaml` 适合本地调试，不发起真实 IPsec / S2b 动作。
